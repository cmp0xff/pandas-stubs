...
def test_pow_py_scalar(left_i: pd.Index) -> None:
    """Test pd.Index[Any] ** Python native scalars"""
    # GH 405
    check(assert_type(left_i**2, pd.Index), pd.Index, np.integer)
    check(assert_type(2**left_i, pd.Index), pd.Index, np.integer)


def test_pow_pd_index(left_i: pd.Index) -> None:
